...
namespace ComputerManager.ServerForm
{
    public partial class ClientPC2Details : Form
    {
        ...
        {
            // Query to fetch logout history from dbo.ClientSessions2
            string query = @"
                SELECT 
                    cs.SessionID,
                    cs.StudentID, 
            ...
            if (connection.State == ConnectionState.Open)
            {
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    using (SqlDataAdapter adapter = new SqlDataAdapter(command))
                    {
                        ...
                        clientPC1Datagridview.DataSource = dataTable; // Bind the data to the DataGridView for history
                    }
                }
            }
            else
        ...
        {
            // Query to get the current user's data for the logged-in session from dbo.ClientSessions2
            string query = @"
                SELECT 
                    cs.SessionID,
                    cs.StudentID, 
            ...
            if (connection.State == ConnectionState.Open)
            {
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    using (SqlDataAdapter adapter = new SqlDataAdapter(command))
                    {
                        DataTable dataTable = new DataTable();
                        adapter.Fill(dataTable);
                        clientPC2Datagridview2.DataSource = dataTable; // Bind the data to the DataGridView for the current session
                    }
                }
            }
            else
